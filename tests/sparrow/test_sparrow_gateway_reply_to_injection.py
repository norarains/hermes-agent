...
def _message_timezone(monkeypatch):
...
async def test_reply_prefix_injected_when_text_absent_from_history():
    ...
    assert result.startswith(
        "04-24 16:36:48: [Replying to Japan is great for culture, food, and efficiency.]"
    )
...
async def test_reply_prefix_still_injected_when_text_in_history():
    ...
    assert result is not None
    assert result.startswith(f"04-24 16:36:48: [Replying to {quoted}]")
    assert result.endswith("What's the best time to go?")
...
async def test_no_prefix_without_reply_context():
    runner = _make_runner()
    source = _source()
    event = MessageEvent(text="hello", source=source, timestamp=_FIXED_TS)
...
async def test_no_prefix_when_reply_to_text_is_empty():
    ...
    assert result == "04-24 16:36:48: hi"
...
async def test_reply_snippet_truncated_to_500_chars():
    ...
    assert result is not None
    assert result.startswith("04-24 16:36:48: [Replying to " + "x" * 500 + "]")
